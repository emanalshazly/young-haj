...
export function QuizStep({ step, stepNumber, totalSteps, onNext }: Props) {
  ...
  return (
    ...
    >
      {/* Progress Line */}
      <div className="w-full flex justify-between items-center mb-8 px-6 text-[#5D4037] font-bold text-2xl bg-[#FFD54F] p-4 rounded-3xl shadow-sm border-4 border-[#FBC02D]">
        <span>المحطة {stepNumber} من {totalSteps}</span>
        <div className="flex-1 mx-6 h-6 bg-white rounded-full overflow-hidden border-2 border-amber-200 relative">
           <div className="absolute inset-0 bg-amber-50 pattern-dots pointer-events-none"></div>
           <motion.div 
        ...
        <span className="text-[#2E7D32] bg-[#E8F5E9] px-4 py-1 rounded-full text-lg border-2 border-[#4CAF50]">{Math.round((stepNumber / totalSteps) * 100)}%</span>
      </div>
      ...
      <div className="bg-white rounded-[40px] shadow-2xl p-8 md:p-12 w-full border-8 border-[#A1887F] flex flex-col items-center text-center relative overflow-hidden">
        ...
        <span className="text-[120px] leading-none font-black text-[#FFB300] opacity-20 block absolute top-4 left-10">
              ...
              <div className={`p-8 rounded-3xl mt-6 border-8 shadow-lg ${isCorrect ? 'bg-[#E8F5E9] border-[#4CAF50] text-[#2E7D32]' : 'bg-[#FFEBEE] border-[#EF5350] text-[#C62828]'}`}>
                <div className="flex items-center justify-center gap-4 mb-4">
                  ...
                  <p className="text-4xl font-black">
                    {isCorrect ? 'إجابة صحيحة يا بطل! 🎉' : 'حاول مرة أخرى! 🤔'}
                  </p>
                </div>
                {isCorrect && (
                  <motion.div 
